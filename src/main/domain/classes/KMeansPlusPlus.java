...
/**
 * Implementación del algoritmo de clustering K-Means con inicialización K-Means++.
 * <p>
 * Esta clase implementa {@link AlgoritmoClustering} y ejecuta el algoritmo K-Means,
 * pero inicializando los centros mediante el método K-Means++ (selección probabilística
 * ponderada por la distancia al centro más cercano ya elegido).
 * <p>
 * El cálculo de distancias y el recálculo de centros sigue la misma filosofía que
 * {@link KMeans}, adaptando el comportamiento al tipo de pregunta ({@link TipoPregunta}).
 */
public class KMeansPlusPlus implements AlgoritmoClustering {
     ...
     */
    public KMeansPlusPlus(int k, int maxIter) {
        this.k = k;
    ...
    /**
     * Inicializa los centros con el método K-Means++.
     * <p>
     * Selecciona el primer centro al azar, y el resto se eligen con probabilidad
     * proporcional a la distancia al cuadrado ({@code D(x)^2}) al centro más cercano
     * ya seleccionado.
     *
     * @param data dataset de entrada
     * @return centros iniciales para el algoritmo
     * @throws IllegalArgumentException si el dataset es vacío o {@code k} es inválido
     */
    Object[][] initializeCenters(Object[][] data) {
        int n = data.length;
        if (n == 0) {
            throw new IllegalArgumentException("Dataset vacío.");
        }
        if (k <= 0) {
            throw new IllegalArgumentException("k debe ser > 0.");
        }
        if (k > n) {
            throw new IllegalArgumentException("k no puede ser mayor que el número de ejemplos.");
        }

        Object[][] centers = new Object[k][];

        // 1) Elegir el primer centro al azar
        int first = random.nextInt(n);
        centers[0] = Arrays.copyOf(data[first], data[first].length);

        // distSquared[i] = distancia^2 del punto i al centro más cercano al actual
        double[] distSquared = new double[n];
        for (int i = 0; i < n; i++) {
            double d = calculateDistance(data[i], centers[0]);
            distSquared[i] = d * d;
        }

        // 2) Elegir el resto de centros
        for (int c = 1; c < k; c++) {
            // 2.1) Sumar todas las distancias^2
            double sum = 0.0;
            for (int i = 0; i < n; i++) {
                sum += distSquared[i];
            }
            ...
            if (sum == 0.0) {
                // Todos los puntos están sobre algún centro ya elegido.
                // Rellenamos los centros que quedan al azar entre los puntos.
                for (int rCenter = c; rCenter < k; rCenter++) {
                    int r = random.nextInt(n);
                    centers[rCenter] = Arrays.copyOf(data[r], data[r].length);
                }
                break;
            }

            // 2.2) Escogemos un punto con probabilidad distSquared[i] / sum
            double r = random.nextDouble() * sum;
            int chosenIndex = 0;
            double acumulado = 0.0;
            for (int i = 0; i < n; i++) {
                acumulado += distSquared[i];
                if (acumulado >= r) {
                    chosenIndex = i;
                    break;
                }
            ...
            // 2.3) Añadimos el nuevo centro
            centers[c] = Arrays.copyOf(data[chosenIndex], data[chosenIndex].length);

            // 2.4) Actualizamos distSquared ya que quizá este nuevo centro está más cerca de algunos puntos
            for (int i = 0; i < n; i++) {
                double d = calculateDistance(data[i], centers[c]);
                double d2 = d * d;
                if (d2 < distSquared[i]) {
